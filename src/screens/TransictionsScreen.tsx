...
const EXCLUDED_SUMMARY_SOURCES = new Set([
  "transfer",
  "group_settlement",
  "goal_contribution",
  "card_payment",
    ...
    return (sectionsQuery.data ?? []).reduce(
      (accumulator, section) => {
        section.data.forEach((item) => {
          if (
            !isAccountView &&
            EXCLUDED_SUMMARY_SOURCES.has(item.sourceType ?? "manual")
          ) {
            return;
          }

          if (item.type === "income") {
            accumulator.income = roundCurrency(
              accumulator.income + item.amount,
            );
          } else {
            accumulator.expense = roundCurrency(
          ...
          <View style={styles.listHeader}>
            ...
            <View style={styles.monthRow}>
              <Pressable
                ...
                style={[
                  styles.monthArrow,
                  allPeriod && styles.monthArrowDisabled,
                ]}
              >
                <ChevronLeft
                  size={18}
                  color={allPeriod ? colors.textSecondary : colors.textPrimary}
                />
              </Pressable>
              <Text style={styles.monthLabel}>
                {allPeriod ? "Todo o período" : monthTitle}
              </Text>
              <Pressable
                accessibilityRole="button"
            ...
            {activeAccountId !== null ? (
              <Text style={styles.filteredTotalsHint}>
                Movimentações da conta selecionada (inclui transferências e

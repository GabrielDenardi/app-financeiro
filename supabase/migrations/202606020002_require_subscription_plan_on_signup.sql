...
as $$
declare
  selected_plan text := coalesce(new.raw_user_meta_data ->> 'subscription_plan', '');
begin
  if selected_plan not in ('basic', 'intermediate', 'pro') then
    raise exception 'Escolha um plano valido para concluir o cadastro.';
  end if;
